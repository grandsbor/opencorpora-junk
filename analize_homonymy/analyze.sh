...
echo 'select distinct tf_id from morph_annot_samples ORDER BY tf_id' | mysql -h $MYSQL_SERV -Dopcorpora > $TMP_TIDS
echo 'select distinct tf_id from morph_annot_candidate_samples ORDER BY tf_id' | mysql -h $MYSQL_SERV -Dopcorpora >> $TMP_TIDS
